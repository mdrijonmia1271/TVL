...
if (!function_exists('ticket_status')) {
    ...
    function ticket_status($status=null)
    {
        $arr = array(
            'P' => 'Pending',
            'W' => 'Working',
        ...
        return empty($status) ? $arr : $arr[$status];
    }
}

if (!function_exists('ticket_status_spv')) {

    function ticket_status_spv()
    {
        $arr = array(
            ''  => 'Select',
            'W' => 'Working',
...
if (!function_exists('base64_image_upload')) {
    function base64_image_upload($file = null, $path, $image_name)
    {

        //file_put_contents('imglog.txt',"inside base64_image_upload " );
        ...
        if (preg_match('/^data:image\/(\w+);base64,/', $file, $type)) {
            ...
            //file_put_contents('imglog.txt',"inside preg_match " );
            if (in_array($type, ['jpg', 'jpeg', 'gif', 'png'])) {
                ...
                if ($data !== false) {
                    ...
                    $upload = file_put_contents($path . '/' . $file_name, $data);
                    //print_r($path . '/' . $file_name);die();
                    //file_put_contents('imglog.txt',"path  ".$path . '/' . $file_name );
                    //file_put_contents('imglog.txt',"\r\n Size: ".$upload );
                    ...
                    $response['msg'] = 'base64_decode failed';

                    //file_put_contents('imglog.txt',"inside  base64_decode failed");
                }

            } else {
                $response['msg'] = 'invalid image type';

                //file_put_contents('imglog.txt',"inside  image type invalid");
            }

        } else {
            $response['msg'] = 'did not match data URI with image data';
        }
...
if (!function_exists('base64_image_convert')) {
    function base64_image_convert($image)
    {
...
function get_customer_status_working($id)
{
    $ci    = &get_instance();
    $sm_db = $ci->load->database('icel', true); // database conection for read operation
...
//             $sms_response =     file_get_contents("http://users.sendsmsbd.com/smsapi?api_key=C20040795d2044a264fe92.94088554&type=text&contacts=$MOBNUMBER&senderid=8804445629107&msg=".$sms_text);
...
function send_sms($phone_number = null, $sms_body = null)
    {
        try {
           ...
           // $sms_response = file_get_contents('http://217.172.190.215/sendtext?apikey=0181285f46e846e9&secretkey=1f5e4eca&callerID=01969910557&toUser='.$MOBNUMBER.'&messageContent=' . $sms_text);
            $sms_response = file_get_contents('https://880sms.com/smsapi?api_key=C200833460afd540e5edb2.44810470&type=text&contacts='.$MOBNUMBER.'&senderid=8809601012469&msg='. $sms_text);
               
            $ci = &get_instance();
            $ci->db->insert('sm_sms_log', [
                'phone' => $phone_number,
                'message' => $sms_body,
                'api_response' => json_encode($sms_response)
            ]);
    ...
    function send_sms_test($phone_number = null, $sms_body = null) {
        ...
        $data = [
            "api_key" => "C200833460afd540e5edb2.44810470",
            "type" => "SMS/unicode",
            // "contacts" =>  $MOBNUMBER,
            "contacts" => "8801723230609",
            // "senderid" => "8809612436108",
            "senderid" => "8809601012469",
            "msg" => $sms_text,
        ];
        $ch = curl_init();
...
function send_push_notification_onsignal($player_id = null, $message = null, $title = null, $app = null)
{
...
function send_push_notification($player_id, $message = '', $title = '', $data = [])
{
    $fields = json_encode([
       'to' => $player_id,
       'notification' => [
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, FCM_APP_URL);
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        'Content-Type: application/json',
        'Authorization: key=' . FCM_APP_ACCESS_KEY,
    ]);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_HEADER, false);
    curl_setopt($ch, CURLOPT_POST, true);
