...
//          $query = $read_db->where("created_by", 'admin');
    ...
    function get_details($auto_id)
    {
        ...
        $read_db->select('ticket.*,machine.*,install_base.insb_version,install_base.insb_serial');
        ...
        $read_db->where("send_to", $engineer_auto_id);
        $read_db->where("srd_id", $auto_id);
        $read_db->where("install_base.insb_customer = ticket.send_from");
...
//        $send_from = $this->session->userdata('send_from');
        ...
        $department   = $this->session->userdata('dep_ref_id');
        $start_date   = $this->session->userdata('start_date');
        $end_date     = $this->session->userdata('end_date');


        if (!empty($department)) {
            $read_db->where('ticket.dep_ref_id', $department);
        }
        if (!empty($ticket_no)) {
