...
class Mod_admin extends CI_Model {
    ...
    function validate_login() {
        ...
        $read_db->select('id,username,password,status,root_admin,user_type');
        $read_db->where('status', 'A');
        $read_db->where('username', $username);
    ...
    function get_all_customer_list_summary($limit, $row) {
        $read_db = $this->load->database('icel', TRUE); /* database conection for read operation */
...
//        $read_db->limit($limit);
    ...
    function customer_total_rows()
    {
        ...
        $sm_db->join('sm_service_request_dtl', 'customer.customer_id = sm_service_request_dtl.send_from', 'inner');
        $sm_db->group_by("customer.customer_id");
        $sm_db->having("customer.status", 'A');
