...
class Mod_common extends CI_Model {
    ...
    function get_customer_array_dropdown(){
        ...
        $query = $read_db->select('customer.customer_id,customer.name,customer.status');

        $read_db->join('sm_service_request_dtl','customer.customer_id = sm_service_request_dtl.send_from','left');

        $read_db->where('customer.status','A');
        $read_db->order_by('customer.name','ASC');
        $read_db->group_by('sm_service_request_dtl.send_from');
