...
class Install_base extends My_Controller
{
    ...
    public function save()
    {
        ...
        if ($_POST) {
            ...
            $department_id = $this->input->post('dep_ref_id');
            ...
            $pmi_date = $this->security->xss_clean($this->input->post('pmi'));


            //============= basic info ==============
            $serial_no = $this->input->post('mcSrial');
            $validate  = $this->Mod_install->validate_serial($serial_no);
            ...
            if (!empty($_FILES['insReport']['name'])) {

                $insReport                    = $this->insReport_do_upload($install_base_id);
                $upload_images['insb_report'] = $insReport;
                $this->db->update('install_base', $upload_images, array('insb_id' => $install_base_id));

            }
            if (!empty($_FILES['accepCer']['name'])) {


                $acceptence                              = $this->accepCer_do_upload($install_base_id);
                $upload_images['insb_acceptence_report'] = $acceptence;
            ...
            if (!empty($_FILES['picture']['name'])) {
                ...
                $picture                  = $this->_do_upload($install_base_id);
            ...
            if ($support == 1 || $support == 2) {
                ...
                if (!empty($pmi_date)) {
                    ...
                    $pmi_data = array_filter($pmi_date);

                    foreach ($pmi_data as $key => $pmi_info) {

                        if (count($pmi_info)) {

                            $date_convert = date("Y-m-d", strtotime($pmi_info));

                            $data = array(
                                'insb_ref_id'    => $install_base_id,
                                'cust_ref_id'    => $customer_id,
                                'mc_ref_id'      => $machine_id,
                                'insb_mc_serial' => $serial_no,
                                'pmi_date'       => $date_convert
                            );

                            $this->db->insert('insb_pmi_trans', $data);
                        }
            ...
            } elseif ($support != 1 && $support != 2 && $support != 3) {
...
//============== install base edit and update ====================================
    ...
    private function _validate($validate)
    {
        $data                 = array();
        ...
        if ($validate == 1) {
            $data['inputerror'][]   = 'mcSrial';
            $data['error_string'][] = 'Serial Number is Already Exist';
            $data['status']         = FALSE;
        }
    ...
    private function insReport_do_upload($master_insert_id)
    {
    ...
    private function accepCer_do_upload($master_insert_id)
    {
    ...
    public function get_by_id($id)
    {
        ...
        $data['pmi']       = $this->Mod_install->get_pmi_by_id($id);
    ...
    public function update()
    {
        ...
        if ($_POST) {
            ...
            if (!empty($_FILES['insReport']['name'])) {
                ...
                $insReport                    = $this->insReport_do_upload($hidden_id);
                $upload_images['insb_report'] = $insReport;
                $this->db->update('install_base', $upload_images, array('insb_id' => $hidden_id));
            ...
            if (!empty($_FILES['accepCer']['name'])) {
                ...
                $acceptence                              = $this->accepCer_do_upload($hidden_id);
                $upload_images['insb_acceptence_report'] = $acceptence;
            ...
            if (!empty($_FILES['picture']['name'])) {
                ...
                $picture                  = $this->_do_upload($hidden_id);
                $upload_images['picture'] = $picture;
                $this->db->update('install_base', $upload_images, array('insb_id' => $hidden_id));
            }
            ...
            if ($support == 1 || $support == 2) {
                ...
                if (!empty($pmi_date)) {
                    ...
                    $this->Mod_install->row_delete($hidden_id);
                    ...
                    foreach ($pmi_data as $key => $pmi_info) {
                        ...
                        if (count($pmi_info)) {
                            ...
                            $updateData = array(
                                'insb_ref_id'    => $hidden_id,
                                'cust_ref_id'    => $customer_id,
                                'mc_ref_id'      => $machine_id,
                                'insb_mc_serial' => $serial_no,
                                'pmi_date'       => $date_convert
                            );
                            $this->db->insert('insb_pmi_trans', $updateData);
                        }
                    }
                }
    ...
    public function get_model()
    {
        ...
        $equ = trim($this->input->post('equ'));

        if (!empty($equ)) {
            $model = $this->Mod_install->get_machine_model($equ);
...
//============================= install base renew ===========================
    ...
    public function ins_list()
    {
        ...
        $data['install']        = $this->Mod_install->install_base_list($this->record_per_page, $row);
        $data['install_export'] = $this->Mod_install->install_base_list_all();
        $data['serial']         = $this->Mod_install->get_serial();

        $config['per_page']    = $this->record_per_page;
        $config['uri_segment'] = '4';
        $config['base_url']    = base_url() . 'sm/install_base/ins_list/';
        $config['total_rows']  = $this->Mod_install->ticket_total_rows();
