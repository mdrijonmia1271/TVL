...
class Manager extends My_Controller
{
        ...
        if (!empty($customer_id)) {
            ...
            if (!empty($machines)) {
                ...
                foreach ($machines as $machine) {
                    ...
                    $version = $machine->insb_version ? ", Version : " . $machine->insb_version : '';
                    $serial  = $machine->insb_serial ? ", Serial : " . $machine->insb_serial : '';

                    $my_machine_name = $machine->mc_name . " ( Model :" . $machine->mc_model . $version . $serial . ")";

                    $options .= "<option value='{$machine->mc_id},{$machine->insb_id},{$machine->insb_serial}'>{$my_machine_name}</option>";
    ...
    {
        $machine_info = $this->input->post('machine_id');
        $customer_id  = $this->input->post('customer_id');

        $result_explode       = explode(',', $machine_info);
        $data['support_type'] = $this->Mod_manager->get_machine_supporttype_info($result_explode[0], $customer_id);
        ...
        $respons = [
            ...
            'department'  => $data['support_type']->dep_ref_id,
            'content'     => $support,
            'machine_id'  => $result_explode[0],
            'insb_id'     => $result_explode[1],
            'insb_serial' => $result_explode[2],
        ];
    ...
    function save()
    {
        ...
        $support_type_id = $this->security->xss_clean($this->input->post('hidden_support_type_id'));
        $customer_id     = $this->security->xss_clean($this->input->post('send_from'));
        $machine_id      = $this->security->xss_clean($this->input->post('machine'));
        $contact_person  = $this->security->xss_clean($this->input->post('cp_number'));
        ...
        $this->form_validation->set_rules('complain_date', 'Complain Date', 'required');
            ...
            if ($support_type_id == 1 || $support_type_id == 2) {
                ...
                if ($valid_support == 1) {
                    ...
                    if (!empty($res_flag)) {
                        ...
                        $customer = $res_flag[1]->customer;
                        $msg      = "Dear Sir/ Madam, Greetings from Tradevision Ltd. Your ticket No. is $res_flag[0]. And our service Eng. will contact you immediately. For details, call 01755645555";

                        $arr['send_me'] = send_sms($contact_person, $msg);
                        /*$ticket = array($contact_person, $customer);

                        foreach ($ticket as $mobile) {

                            $arr['send_me'] = send_sms($mobile, $msg);
                        }*/
    ...
    function ticketsearch()
    {
        ...
        $arr                  = array(
            ...
            'total_ticket' => is_array($results) ? count($results) : 0
        );
    ...
    public function ticket_cancel($id)
    {
        $this->Mod_manager->delete_by_id($id);
